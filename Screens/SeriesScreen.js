...
import { Tv, Star } from "lucide-react-native";
import { getPopularSeries, getSeriesByGenre, getImageUrl } from "../services/api";
...
export default function SeriesScreen({ navigation }) {
  ...
  const fetchSeries = useCallback(async (genreId, pageNum = 1, append = false) => {
    setLoading(true);
    try {
      let results = [];
      if (genreId === 0) {
        results = await getPopularSeries(pageNum);
      } else {
        results = await getSeriesByGenre(genreId, pageNum);
      }
  ...
  useEffect(() => {
    fetchSeries(selectedGenre, 1, false);
    setPage(1);
  }, [selectedGenre, fetchSeries]);

  const onRefresh = async () => {
    setRefreshing(true);
    await fetchSeries(selectedGenre, 1, false);
    setPage(1);
    setRefreshing(false);
  };
  ...
  const loadMore = () => {
    if (!loading) {
      ...
      setPage(nextPage);
      fetchSeries(selectedGenre, nextPage, true);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTitleRow}>
          <Tv color="#FFFFFF" size={24} style={{ marginRight: 10 }} />
          <Text style={styles.headerTitle}>TV Series</Text>
        </View>
        <Text style={styles.headerSubtitle}>Binge-watch top series and seasons</Text>
      </View>
      ...
      <View style={styles.genreListContainer}>
        <FlatList
          ...
          data={SERIES_GENRES}
          keyExtractor={(item) => `tv-genre-${item.id}`}
